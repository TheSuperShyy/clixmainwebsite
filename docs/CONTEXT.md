...
- **[--:--]** `nav` — **The banner ticker got its graph back, measuring the one thing it honestly can.** User asked for a small green/red up-down chart beside each token price. **Half of that is not buildable honestly and it was said before building, not after**: OpenRouter returns the current list price and nothing else, there is no history endpoint and no database, so "up or down" has no number behind it — which is why the old stock sparkline came off on 2026-08-08. Three options went to the user with the constraint stated; **user picked field-position, and picked CHEAPER = GREEN**. `PriceRank` is a **30x12px column chart of the strip's own nine prices**, sorted cheap-to-dear, this row's column lit — identical skyline every row so the reader gets one fixed reference frame. ⚠️ **THE LOG SCALE WAS COMPUTED, NOT PREFERRED**: the field spans **$0.90 → $35.00 (39.1x)** and linearly the six cheapest models occupy **2.4px of twelve** — seven stubs beside two tall bars, saying only what the price already says. ⚠️ **Median came back $8.00 with a tie ON it**, so `total <= median` is load-bearing — 5 green / 4 red where `<` would call the median itself dear. Scalar is `input + output` (both figures already on the row); a weighted blend was rejected as asserting an unmeasured usage pattern. Two tokens restored at their 2026-08-08 measured values, **AA on `banner` at 10.6:1 / 6.4:1**, but **RENAMED `price-low`/`price-high`, never `up`/`down`** — the meaning inverted, and a call site reaching for the familiar word would get a backwards chart. **Colour is not the only channel** (height carries the same order). Free models excluded from min/max so `Math.log` cannot hit -Infinity; <2 models renders no chart. **RTL: deliberately NOT mirrored** — the columns are a sorted axis beside an already-LTR-isolated price. New `chrome.a11y.tickerRank`; ⚠️ **the Hebrew is authored and unread by a native speaker**. Build + `tsc` + eslint clean. **Not viewed in a browser** — handed over. → [detail](../features/nav/CONTEXT.md)
