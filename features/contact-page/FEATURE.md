...
| Consent line | reference links Privacy + Terms, beside a checkbox | ✅ **RESOLVED 2026-08-18** — a real `<input type="checkbox">` whose label links both | Both routes were built on 2026-08-16, so the 404 objection that kept this plain text is gone. **Question closed.** ⚠️ Those two links are also why the form now drafts itself to `sessionStorage` (`clix-contact-draft.v1`) — a client-side navigation unmounts the form, so leaving to read either document used to lose every field. Restored via `useSyncExternalStore` (an effect is both forbidden by this repo's lint and a hydration mismatch on a prerendered page), re-validated on the way out of storage, and deleted on a successful send. The sentence is one template per locale carrying `{privacy}`/`{terms}`, not a fixed five-run split — English names the privacy policy first and the reference's Hebrew names תנאי השימוש first. ⚠️ It is **not** wrapped in a `<label>`: a label containing an anchor toggles the box when the anchor is clicked, so the input is named with `aria-labelledby`. Send is `aria-disabled` — not `disabled` — until every required field validates and the box is ticked; see `CONTEXT.md` for why that distinction is load-bearing. |
